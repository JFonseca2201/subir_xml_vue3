...
<template>
  <div class="pa-4 pa-sm-6 roles-management-page">
          ...
          <VTable hover class="roles-table">
            <thead>
              <tr>
                ...
                <th class="text-left font-weight-bold text-uppercase" style="min-width: 300px;">
                  PERMISOS
                </th>
              ...
              <tr v-for="item in list_roles" :key="item.id" class="roles-row align-middle">
                ...
                <td class="text-left py-3">
                  <div class="d-flex align-center gap-3">
                    ...
                    <span class="font-weight-semibold text-body-1 text-grey-darken-4 text-capitalize">
                      {{ item.name }}
                    </span>
                  </div>
                </td>
                ...
                <td class="text-left py-3">
                  <div v-if="!item.permissions_pluck || item.permissions_pluck.length === 0"
                  ...
                  <div v-else class="d-flex flex-wrap gap-2 align-center">
                    <VChip v-for="(permission, index) in item.permissions_pluck.slice(0, 4)" :key="index"
                      ...
                      variant="flat" size="small" class="text-capitalize elevation-1 font-weight-medium">
                      {{ (typeof permission === 'object' ? permission.name :
                        permission).replace('_', ' ') }}
                    </VChip>

                    <VChip v-if="item.permissions_pluck.length > 4" variant="tonal" size="small" color="secondary"
                      ...
                      <VTooltip activator="parent" location="top" max-width="350">
                        <div class="pa-2">
                          <div class="text-subtitle-2 font-weight-bold mb-2 pb-1"
                          ...
                          <div class="d-flex flex-wrap gap-1">
                            <VChip v-for="(permission, index) in item.permissions_pluck.slice(4)" :key="index"
                              :color="permissionColor(typeof permission === 'object' ? permission.name : permission)"
                              variant="flat" size="x-small" class="text-capitalize font-weight-medium">
                              {{ (typeof permission === 'object' ? permission.name :
                                permission).replace('_', ' ') }}
                            </VChip>
                          </div>
                ...
                <td class="text-no-wrap text-center py-3">
                  <div class="d-flex justify-center align-center">
                    <VBtn v-if="item.id !== 1 && can('edit_role')" class="action-btn" variant="text" icon size="small"
                      color="primary" title="Editar Rol" @click="editItem(item)">
                      <VIcon icon="ri-pencil-line" size="20" />
                    </VBtn>
                    <VBtn v-if="item.id !== 1 && can('delete_role')" class="action-btn" variant="text" icon size="small"
    ...
    <RoleAddDialog v-model:isDialogVisible="isRoleAddDialogVisible" @add-role="addNewRole" />
    <RoleEditDialog v-if="role_selected_edit && isRoleEditDialogVisible"
      v-model:isDialogVisible="isRoleEditDialogVisible" :role-selected="role_selected_edit" @edit-role="addEditRole" />
    <RoleDeleteDialog v-if="role_selected_delete && isRoleDeleteDialogVisible"
      v-model:isDialogVisible="isRoleDeleteDialogVisible" :role-selected="role_selected_delete"
