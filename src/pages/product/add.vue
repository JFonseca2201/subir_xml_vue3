        ...
        <VCol cols="12" lg="8">
            ...
            <VCardItem class="bg-white py-3 px-4 border-b">
              <template #title>
                <div class="d-flex align-center gap-2">
                  <VIcon icon="ri-shapes-line" size="18" class="text-medium-emphasis" />
          ...
          <VCard class="rounded-xl border-light elevation-1 mb-6 overflow-hidden">
            ...
            <VCardText class="pa-4 pa-sm-5 bg-white">
              <VRow>
                ...
                <VCol v-if="skuExistsAlert" cols="12" class="py-1">
                  <VAlert :type="skuExistsAlert.type || 'error'" variant="tonal" density="comfortable" closable
                    class="rounded-lg mb-1" @click:close="skuExistsAlert = null">
                ...
                <!-- Marca -->
                <VCol v-if="product.item_type !== '2'" cols="12" sm="6">
                  <VCombobox v-model="product.brand" :items="brandOptions" :rules="brandRules" label="Marca"
                    placeholder="Selecciona o escribe una marca (Ej. MONROE)" variant="outlined" density="comfortable"
                    prepend-inner-icon="ri-building-line" hide-details="auto" clearable color="primary" />
                </VCol>

                <!-- Usos / Aplicaciones -->
                <VCol cols="12" :sm="product.item_type !== '2' ? 6 : 12">
                  <VTextField v-model="product.uses" label="Usos / Aplicaciones"
                    placeholder="Ej. VEHÍCULO LIVIANO / CAMIONETAS" variant="outlined" density="comfortable"
              ...
              <VRow>
                ...
                <VCol cols="12" sm="6">
                  <VSelect v-model="product.product_categorie_id" :items="categories" item-title="title" item-value="id"
                    ...
                    placeholder="Selecciona categoría" prepend-inner-icon="ri-folder-3-line" hide-details="auto"
                    required color="primary" :loading="isLoading" />
                </VCol>
          ...
          <VCard class="rounded-xl border-light elevation-1 mb-6 overflow-hidden">
            <VCardItem class="bg-white py-3 px-4 border-b">
              <template #title>
                <div class="d-flex align-center gap-2">
                  <VIcon icon="ri-file-text-line" size="18" class="text-medium-emphasis" />
            ...
            <VCardText class="pa-4 bg-white">
              <VTextarea v-model="product.notes"
                placeholder="Notas adicionales, especificaciones técnicas o términos de garantía..." variant="outlined"
                rows="2" density="comfortable" hide-details="auto" color="primary" auto-grow />
            </VCardText>
          </VCard>
        </VCol>
              ...
              <VCardText class="pa-4 bg-white">
                <div ref="dropZoneRef" class="cursor-pointer" @click="() => open()">
                  <div v-if="fileData.length === 0"
                    ...
                    <VAvatar size="38" color="primary" variant="tonal" class="rounded-circle">
                      <VIcon icon="ri-upload-cloud-2-line" size="20" />
                    </VAvatar>
                    <div>
                  ...
                  <div v-else class="pa-2 border rounded-lg bg-slate-50">
                    <div v-for="(item, index) in fileData" :key="index" class="text-center">
                      <VImg :src="item.url" height="150px" class="rounded-lg mb-2 mx-auto bg-white border" contain />
                      <div class="text-caption font-weight-semibold text-truncate mb-2 text-slate-800">
                        {{ item.file.name }}
                      </div>
                      <VBtn variant="tonal" block size="small" color="error" prepend-icon="ri-delete-bin-line"
            ...
            <VCard class="rounded-xl border-light elevation-1 overflow-hidden">
              ...
              <VCardText class="pa-4 bg-white d-flex flex-column gap-4">
                  ...
                  <VRow dense>
                    <VCol cols="12" sm="6">
                      <VTextField v-model="priceSaleWithIva" :rules="priceRules" label="PVP (Con IVA) *"
                        ...
                        type="number" step="0.01" min="0" required color="primary" class="font-mono font-weight-bold" />
                    </VCol>
                    <VCol cols="12" sm="6">
                      <VTextField v-model="product.price_sale" :rules="priceRules" label="PVP Base (Sin IVA) *"
                        placeholder="0.00" variant="outlined" density="comfortable" prefix="$" hide-details="auto"
                        type="number" step="0.01" min="0" required color="primary" class="font-mono" />
                    </VCol>
                  </VRow>
                </div>
                  ...
                  <VRow dense>
                    <VCol cols="12" sm="6">
                      <VTextField v-model="purchasePriceWithIva" :rules="priceRules" label="Costo (Con IVA)"
                        placeholder="0.00" variant="outlined" density="comfortable" prefix="$" hide-details="auto"
                        type="number" step="0.01" min="0" color="primary" class="font-mono" />
                    </VCol>
                    <VCol cols="12" sm="6">
                ...
                <div
                  ...
                  <VChip color="success" variant="tonal" size="small" class="font-weight-bold font-mono">
            ...
            <VCard class="rounded-xl border-light elevation-1 overflow-hidden">
              <VCardItem class="bg-white py-3 px-4 border-b">
                <template #title>
                  <div class="d-flex align-center gap-3">
                    <VAvatar size="36" color="info" variant="tonal" class="rounded-lg">
                      <VIcon icon="ri-check-double-line" size="20" />
                    </VAvatar>
                    <div>
                      <h3 class="text-subtitle-1 font-weight-bold text-slate-900 mb-0">
                        Confirmación
              ...
              <VCardText class="pa-4 bg-white d-flex flex-column gap-3">
                ...
                <VAlert v-if="warning" color="warning" variant="tonal" class="w-100 mb-1 rounded-lg" border="start"
                  ...
                  <div class="d-flex align-center">
                    <VIcon icon="ri-alert-line" class="mr-2" size="18" />
                ...
                <VAlert v-if="error_exist" color="error" variant="tonal" class="w-100 mb-1 rounded-lg" border="start"
                  ...
                  <div class="d-flex align-center">
                    <VIcon icon="ri-error-warning-line" class="mr-2" size="18" />
                ...
                <div class="d-flex gap-3 w-100 pt-1">
                  <VBtn color="secondary" variant="outlined" prepend-icon="ri-close-line"
                    class="font-weight-medium flex-grow-1" size="large" :disabled="isLoading || loader.loading"
                    @click="router.push(backRoute)">
                    Cancelar
                  </VBtn>

                  <VBtn type="submit" color="primary" variant="elevated" prepend-icon="ri-save-3-line"
                    class="font-weight-bold flex-grow-1" size="large" :loading="loader.loading"
