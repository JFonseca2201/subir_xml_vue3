...
<template>
    <VDialog :model-value="props.isDialogVisible" @update:model-value="val => emit('update:isDialogVisible', val)"
        max-width="600" persistent transition="dialog-bottom-transition">
        <VCard class="rounded-xl">
            <VCardTitle class="d-flex align-center justify-space-between pa-4">
                <div class="d-flex align-center gap-2">
                    <VIcon size="24" color="success">ri-file-excel-2-line</VIcon>
                    <span class="text-h6 font-weight-bold">Importar Productos</span>
                </div>
                <VBtn icon variant="text" @click="closeDialog" :disabled="loader.loading">
            ...
            <VCardText class="pt-6">
                ...
                <VFileInput accept=".xlsx, .xls, .csv" label="Seleccionar archivo Excel" variant="outlined" clearable
                    prepend-inner-icon="ri-upload-cloud-2-line" @change="loadFile($event)" :disabled="loader.loading" />
                <VAlert v-if="error_exist" type="error" variant="tonal" class="mt-4">
                    {{ error_exist }}
                </VAlert>
            </VCardText>
            <VDivider />
            <VCardActions class="pa-4 justify-end gap-3">
                <VBtn variant="tonal" color="error" @click="closeDialog" :disabled="loader.loading">
                    <VIcon left>ri-close-circle-line</VIcon> &nbsp; Cancelar
                </VBtn>
                <VBtn variant="elevated" color="success" @click="importExcel" :loading="loader.loading">
                    <VIcon left>ri-file-excel-2-line</VIcon> &nbsp; Importar Data
                </VBtn>
            </VCardActions>
        </VCard>
    </VDialog>
