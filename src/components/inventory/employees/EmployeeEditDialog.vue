...
// Emits
const emit = defineEmits(['update:modelValue', 'employee-updated'])
...
const updateEmployee = async () => {
    ...
    try {
        ...
        const response = await $api(`employees/${employeeForm.value.id}`, {
            method: 'PUT',
            body: JSON.stringify({
        ...
        if (response.status === 200) {
            ...
            // Emitir evento de empleado actualizado
            emit('employee-updated', response.employee)
    ...
    <VDialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="800">
        <VCard>
            ...
            <VCardText class="pa-4">
                <VForm ref="employeeFormRef" @submit.prevent="updateEmployee">
            ...
            <VCardActions class="pa-4">
                ...
                <VBtn color="primary" variant="elevated" @click="updateEmployee" prepend-icon="ri-save-3-line">
                    Actualizar Empleado
                </VBtn>
            </VCardActions>
        </VCard>
