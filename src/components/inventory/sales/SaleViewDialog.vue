...
<template>
    <VDialog max-width="1000" :model-value="props.isDialogVisible" @update:model-value="closeDialog" persistent>
        <VCard class="pa-0" elevation="8">
            <!-- Header con gradiente -->
            <VCardTitle class="pa-6 text-center position-relative bg-gradient-to-r from-primary to-primary-darken-1" dark>
                ...
                <div class="d-flex flex-column align-center mb-3">
                    <VIcon icon="ri-file-list-3-line" size="48" class="mb-2" />
                ...
                <!-- Chip de estado -->
                <VChip :color="getStatusColor" variant="elevated" size="small">
                    <VIcon start :icon="saleData.status === 'completed' ? 'ri-checkbox-circle-line' : saleData.status === 'pending' ? 'ri-time-line' : 'ri-close-circle-line'" />
                    {{ getStatusLabel }}
                </VChip>
            </VCardTitle>
            ...
            <VCardText class="pa-6">
                <VRow>
                    ...
                    <VCol cols="12" md="6">
                        <VCard class="pa-4 h-100" elevation="2" rounded="lg">
                            <VCardTitle class="d-flex align-center pa-0 mb-4">
                                <VIcon icon="ri-file-info-line" color="primary" class="me-2" />
                            ...
                            <VRow dense>
                                <VCol cols="12">
                                    <div class="mb-3">
                                        ...
                                        <VChip :color="getDocumentTypeColor" size="small" variant="tonal">
                                            {{ getDocumentTypeLabel }}
                                        </VChip>
                                    </div>
                                </VCol>
                                ...
                                <VCol cols="12">
                                    <div class="mb-3">
                                        ...
                                        <div class="text-body-1 font-weight-medium">{{ saleData.document_number }}</div>
                                    </div>
                                </VCol>

                                <VCol cols="12">
                                    <div class="mb-3">
                                        <div class="text-caption text-medium-emphasis mb-1">Fecha de Servicio</div>
                                        <div class="d-flex align-center">
                                            <VIcon icon="ri-calendar-line" size="16" class="me-1 text-primary" />
                                ...
                                <VCol cols="6">
                                    <div class="mb-3">
                                        <div class="text-caption text-medium-emphasis mb-1">Estado de Pago</div>
                                        <VChip :color="getPaymentStatusColor" size="small" variant="tonal">
                                ...
                                <VCol cols="6">
                                    <div class="mb-3">
                                        ...
                                        <div class="text-body-2">{{ saleData.payment_method || 'No especificado' }}</div>
                                    </div>
                                </VCol>

                                <VCol cols="12" v-if="hasPaymentDistributions">
                                    <div class="mb-3">
                                        <div class="text-caption text-medium-emphasis mb-1">Detalle de Pagos</div>
                                        <div class="border rounded-lg overflow-hidden mt-2">
                                            <VTable class="text-no-wrap" density="compact">
                                                ...
                                                <tbody>
                                                    <tr v-for="(dist, index) in getPaymentDistributions" :key="index">
                                                        <td class="pa-2 text-center font-weight-medium">{{ index + 1 }}</td>
                                                        <td class="pa-2">{{ dist.payment_method }}</td>
                                                        <td class="pa-2">{{ dist.account?.name || 'Cuenta ' + dist.account_id }}</td>
                                                        <td class="pa-2 text-right font-weight-bold">{{ formatCurrency(dist.amount) }}</td>
                                                    </tr>
                                                </tbody>
                                            </VTable>
                                        </div>
                                ...
                                <VCol cols="12">
                                    <div class="mb-3">
                                        <div class="text-caption text-medium-emphasis mb-1">Kilometraje</div>
                                        <div class="d-flex align-center">
                                            <VIcon icon="ri-speed-up-line" size="16" class="me-1 text-info" />
                    ...
                    <VCol cols="12" md="6">
                        <VCard class="pa-4 h-100" elevation="2" rounded="lg">
                            <VCardTitle class="d-flex align-center pa-0 mb-4">
                                <VIcon icon="ri-user-vehicle-line" color="success" class="me-2" />
                            ...
                            <VRow dense>
                                <VCol cols="12">
                                    <div class="mb-3">
                                        ...
                                        <div class="d-flex align-center">
                                            <VIcon icon="ri-user-line" size="16" class="me-1 text-success" />
                                ...
                                <VCol cols="12">
                                    <div class="mb-3">
                                        ...
                                        <div class="d-flex align-center">
                                            <VIcon icon="ri-id-card-line" size="16" class="me-1 text-info" />
                                ...
                                <VCol cols="12">
                                    <div class="mb-3">
                                        <div class="text-caption text-medium-emphasis mb-1">Vehículo</div>
                                        <div v-if="saleData.vehicle" class="d-flex align-center">
                                            <VIcon icon="ri-car-line" size="16" class="me-1 text-warning" />
                                            <VChip size="small" variant="outlined" color="primary" class="font-weight-bold">
                                                {{ getVehicleLicensePlate }}
                                            </VChip>
                                        </div>
                                        <span v-else class="text-body-2 text-medium-emphasis">No especificado</span>
                                    </div>
                                </VCol>

                                <VCol cols="12" v-if="saleData.vehicle">
                                    <div class="mb-3">
                                        <div class="text-caption text-medium-emphasis mb-1">Información del Vehículo</div>
                                        <div class="text-body-2">{{ getVehicleInfo }}</div>
                                    </div>
                                </VCol>
                            </VRow>
                        </VCard>
                    ...
                    <VCol cols="12">
                        <VCard class="pa-4" elevation="2" rounded="lg">
                            <VCardTitle class="d-flex align-center pa-0 mb-4">
                                <VIcon icon="ri-shopping-cart-2-line" color="warning" class="me-2" />
                                <span class="text-h6 font-weight-bold">Detalle de Productos/Servicios</span>
                            </VCardTitle>

                            <div v-if="saleData.details && saleData.details.length > 0" class="border rounded-lg overflow-hidden">
                                <VTable class="text-no-wrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, index) in saleData.details" :key="index">
                                            <td class="pa-2">{{ item.description }}</td>
                                            <td class="pa-2">{{ item.quantity }}</td>
                                            <td class="pa-2">{{ formatCurrency(item.price) }}</td>
                                            <td class="pa-2">{{ formatCurrency(item.discount || 0) }}</td>
                                            <td class="pa-2 text-right font-weight-bold">{{ formatCurrency(item.total) }}</td>
                                        </tr>
                                    </tbody>
                                </VTable>
                            </div>
                    ...
                    <VCol cols="12" md="6">
                        <VCard class="pa-4" elevation="2" rounded="lg">
                            <VCardTitle class="d-flex align-center pa-0 mb-4">
                                <VIcon icon="ri-file-text-line" color="info" class="me-2" />
