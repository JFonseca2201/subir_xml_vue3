...
const emit = defineEmits(['update:isDialogVisible', 'addEmployee'])
...
const saveEmployee = async () => {
    ...
    try {
        console.log('Datos del empleado a guardar:', employeeForm.value);
        ...
        const resp = await $api("employees", {
            ...
            onResponseError({ response }) {
                if (response.status === 422) {
                    ...
                    error.value = response._data?.message || 'Error al guardar empleado';
                }
        ...
        if (resp.status === 201) {
            success.value = resp.data?.message || 'Empleado creado exitosamente';
            ...
            setTimeout(() => {
                ...
                emit('addEmployee', updatedData);
            }, 1500);
        } else {
            error.value = resp.data?.message || 'Error al guardar empleado';
            showNotification(error.value, 'error');
        }
    } catch (error) {
        console.error('Error al guardar empleado:', error);
        error.value = 'Error al guardar empleado. Intente nuevamente.';
        showNotification('Error al guardar empleado. Intente nuevamente.', 'error');
    } finally {
        loading.value = false;
    }
}
    ...
    <VDialog max-width="800" v-model="props.isDialogVisible" persistent>
        <VCard class="pa-sm-10 pa-5">
            ...
            <VCardText class="text-center pb-6">
                ...
                <h4 class="text-h4 font-weight-bold mb-1">Nuevo Empleado</h4>
            ...
            <!-- Form -->
            <VForm ref="formRef" @submit.prevent="saveEmployee">
                <VRow dense>
                    <!-- Datos Personales -->
                    <VCol cols="12">
